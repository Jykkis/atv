...
public:
  ...
  void callback(const sensor_msgs::ImageConstPtr& cata_image_ptr)
  {
    ...
    cv::Mat m = cv::cvarrToMat(&dst_pan_, false); // memory leak happens here
...
z
    //ROS_INFO("encoding: %s", cv_ptr->encoding.c_str());
    //ROS_INFO("cv_ptr width: %d", temp.width);
    //ROS_INFO("cv_ptr height: %d", temp.height);
    //ROS_INFO("cv_ptr nSize: %d", temp.nSize);
