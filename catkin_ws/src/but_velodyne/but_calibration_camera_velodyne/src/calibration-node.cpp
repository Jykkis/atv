...
{
  ROS_INFO("Starting calibration_node main()");
//  ROS_INFO("OpenCV: %s", cv::getBuildInformation().c_str());
...
//  ROS_INFO_STREAM("Camera frame:" << CAMERA_FRAME_TOPIC); 
//  ROS_INFO_STREAM("Camera info:" << CAMERA_INFO_TOPIC);
//  ROS_INFO_STREAM("Velodyne:" << VELODYNE_TOPIC);
  ROS_INFO_STREAM("DoRefinement:" << doRefinement);
